"""
Contains functions to handle comments and reviews in database
"""
from db import db
...
def add_review(title, username, game_id, review, rating):
    ...
    try:
        sql = text(
            "INSERT INTO reviews "
            "(title, username, game_id, review, rating) "
            "VALUES (:title, :username, :game_id, :review, :rating)")
        print(sql)
        db.session.execute(sql,
            {"title":title, "username":username, 
             "game_id":game_id, "review":review, "rating":rating})
        db.session.commit()
        return True
    except:
        return False
...
def get_reviews(game_id):
    """Get reviews for a game from the database."""
    try:
...
def add_comment(username, review_id, comment):
    ...
    try:
        sql = text(
            ...
            "(username, review_id, comment) VALUES (:username, :review_id, :comment)")
...
def get_comments(review_id):
    """Get comments for a review from the database."""
    try:
