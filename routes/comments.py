...
def add_review_route(id):
    ...
    if request.method == "POST":
        ...
        if int(rating) < 1 or int(rating) > 10:
            return render_template("error.html", message="Rating must be between 1 and 10")
        elif len(review) < 1 or len(title) < 1:
            return render_template(
                "error.html", message="Review or title cannot be empty")
        elif add_review(title, username, id, review, rating):
            return redirect("/game/" + str(id))
        else:
            return render_template(
                "error.html", message="You have already reviewed this game")
...
def comments_route(id):
    ...
    if request.method == "GET":
        if get_comments(id):
            ...
            return render_template("error.html", message="Comments not found")
    if request.method == "POST":
        username = session["username"]
        ...
        if add_comment(username, id, comment):
            ...
            return render_template("error.html", message="Adding comment failed")
